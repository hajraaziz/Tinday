...
export function AIMessageBubble({ message, streaming }: AIMessageBubbleProps) {
  ...
  return (
    <div className="flex justify-start gap-2.5">
      ...
      <div className="max-w-[80%] rounded-2xl rounded-bl-md bg-[#221E30] text-white px-4 py-2.5 break-words">
        {message.content && (
          <MarkdownMessage content={message.content} streaming={streaming} />
        )}
        {streaming && (
          <span className="inline-block w-1.5 h-4 ml-0.5 -mb-0.5 bg-[#8478D4] animate-pulse align-text-bottom" />
        )}
      </div>
    </div>
  );
